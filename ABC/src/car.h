...
namespace tool{
    ...
    public:
        ...
        //给定一个位置找最近的障碍物
        Obstacle *getNearestObs(double x,double y);

        //给定一个位置，找一定距离内的障碍物，障碍物个数通过size参数返回,返回结果用完后要自行释放内存
        Obstacle **getObsRange(double x,double y,double dis,int & size);
    };
    ...
    public:
        ...
        int calcIndex(Path path,double k=0.1,double dis = 2.0 );
    };
